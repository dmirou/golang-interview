...
func TestFindUpperBound(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Two elements - target between",
			arr:      []int{2, 4},
			target:   3,
			expected: -1,
		},
		...
		{
			name:     "Two consecutive duplicates",
			arr:      []int{1, 2, 2, 3, 4},
			target:   2,
			expected: 2,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := findUpperBound(tt.arr, tt.target)
			assert.Equal(t, tt.expected, result, "findUpperBound(%v, %d) = %d, want %d", tt.arr, tt.target, result, tt.expected)
		})
	}
}
